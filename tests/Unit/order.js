        ...
        jQuery.ajax({
            ...
            success: function (consigneeResult, textStatus) {
                ...
                if (consigneeResult.success) {
                    ...
                    if ($('#allHaoKa').val() == "true") {
                        $("#part-inv").html("<span class='mr10'>" + $("#allHaoKaTip").val() + "&nbsp;</span>");
                    } else {
                        ...
                        if (consigneeResult.resetHK) {
                            $("#part-inv").html("<span class='mr10'>" + $('#qqsInvoice').val() + "&nbsp;</span><a onclick='edit_Invoice()' id='invoiceEdit' class='ftx-05 invoice-edit hide' href='#none'>修改</a>");
                        }
...
//		  			    	  pickSiteListHTML = pickSiteListHTML + jsonO.pickName+"   ";
    ...
    function use_NewConsignee() {
        if (checkMaxConsigneeSize()) {
            showLargeMessage('地址限制', $('#maxLimitAddr').val() || '');
            return;
        }
        $('body').dialog({
            title: '新增收货人信息',
...
// *****************************************************发票开始********************************************************************
    ...
    function edit_Invoice() {
        var hasHaoKa = $("#hasHaoKa").val();
        var hasHyj = $("#hasHyj").val();
        var url = OrderAppConfig.DynamicDomain + "/invoice/editInvoice.action?invoiceParam.hasHaoKa=" + hasHaoKa + "&invoiceParam.hasHyj=" + hasHyj;
     ...
     * @param _id
     */
    function delete_Invoice(id, type) {
        $('#mainId').hide();
        ...
        var _$this = _tempWindow.$('#' + type + '-invoice-tit-list .invoice-item[date-fid=fid' + id + ']');
                    ...
                    jQuery.ajax({
                        ...
                        success: function (dataResult, textStatus) {
                            if (isHasMessage(dataResult)) {
                                ...
                                if (_$this.siblings().length > 1) {
                                    _$this.siblings().eq(0).click();
                                } else {
                                    if (type == 'danwei') {
                                        _tempWindow.$('#nor_code').val("");
                                        _tempWindow.$('#nor_address').val("");
                                        _tempWindow.$('#nor_phone').val("");
                                        _tempWindow.$('#nor_bankName').val("");
                                        _tempWindow.$('#nor_bankAccount').val("");
                                        _tempWindow.$('#nor_more_info_box').hide();
                                        _tempWindow.$('#nor_more_info span').html("填写单位地址、电话、开户银行及账号");
                                        _tempWindow.$('#nor_more_info_id span').html("填写单位地址、电话、开户银行及账号");
                                        _tempWindow.$('#danwei-invoice-tit-list').hide();
                                        _tempWindow.$('#danwei-invoice-tit-list').hide();
                                    }
                                }
                                _$this.remove();
                                var len = _tempWindow.$('#' + type + '-invoice-tit-list').find('.invoice-item').length;
                                if (len < 11) {
                                    _tempWindow.$('#' + type + '-add-invoice').show();
                                } else {
                                    _tempWindow.$('#' + type + '-add-invoice').hide();
                                }
                                diaDel.close();
                                $('#mainId').show();
                            }
...
//            var hasJdExtFreight = 0;
    ...
    function flushOrderPrice(orderPrice, isFlushSkuList, typeFlag) {
        ...
        // 合约机优惠券
        if (orderPrice.hyjDiscount != null && +orderPrice.hyjDiscount > 0) {
            $("#hyjDiscountId").text("-￥" + orderPrice.hyjDiscount.toFixed(2));
            $("#showHyjDiscount").css("display", "block");
            $("#hyjDiscountId").css("display", "block");
            $("#showPresaleHyjDiscount").css("display", "block");
            $("#presaleHyjDiscountId").text("-￥" + orderPrice.hyjDiscount.toFixed(2));
        } else {
            $("#showHyjDiscount").css("display", "none");
            $("#hyjDiscountId").css("display", "none");
            $("#showPresaleHyjDiscount").css("display", "none");
        }
     ...
     * 构造密码框 - 新版
     */
    function needPaymentPasswordNew(result) {
        if (result.need) {
            PwComponent.reset();
            shortPwdActive = result.shortPwdActive;
            if (result.shortPwdActive) {
                $("#paypasswordPanel").show();
                $("#no-pwd-error").hide();
                $("#pwd-error").hide();
            } else {
                PwComponent.switch(true)
                $("#paypasswordPanel").show();
            }
        } else {
    ...
    window.needPaymentPasswordNew = needPaymentPasswordNew;
     ...
     * 构造密码框 - 旧版
     */
    function needPaymentPasswordOld(result) {
        if (result.need) {
            $("#txt_paypassword").val("");
            if (result.shortPwdActive) {
                resetPaypassword();
    ...
    window.needPaymentPasswordOld = needPaymentPasswordOld;
        ...
        jQuery.ajax({
            ...
            success: function (result) {
                ...
                if (isHasMessage(result)) {
                    ...
                    if (paypwdNewVersion) {
                        needPaymentPasswordNew(result);
                    } else {
                        needPaymentPasswordOld(result);
                    }
                }
            }
        });
        ...
        jQuery.ajax({
            ...
            success: function (dataResult, textStatus) {
                ...
                if (dataResult.needPayPwd) {
                    shortPwdActive = dataResult.shortPwdActive
                    if (paypwdNewVersion) {
                ...
                if (dataResult.balance && dataResult.balance.success && dataResult.balance.checked) {
                    ...
                    if (dataResult.balance.leavyMoney > 0) {
                        $('#balanceitem').removeClass('hide');
                        var html = template('virtual_balance_tmp', {
                            'balance': dataResult.balance.leavyMoney,
                            'checked': dataResult.balance.checked
                        });
                        $("#jdBalance").html(html);
                    }
                }
                if (window.virtual) {
                    window.virtual.setAvailableNumbers(dataResult);
                }
                // 京豆优惠购是否足额和京豆优惠购商品id
...
// 判断是否加载验证码
    ...
    function obtainCopyInfoConfig(consignee_provinceId, consignee_cityId) {
        ...
        var param = "orderInfoParam.provinceId=" + provinceId + "&orderInfoParam.cityId=" + cityId + "&orderInfoParam.countyId=" + countyId + "&orderInfoParam.townId=" + townId;
        param = addFlowTypeParam(param);
        jQuery.ajax({
            type: "POST",
            dataType: "json",
            ...
            success: function (result) {
                ...
                if (copyInfoList[2].isUsed == 1) {
                    ...
                    $("#justPick").val(copyInfoList[2].justPick);
                } else {
 ...
 */
    ...
    function submit_Order(enterPriseUser, agreement, jxj) {
        ...
        if (($("#taiWanIdCardCheck").val() == "0" && typeof (selectedProvinceid) !== 'undefined' && selectedProvinceid == $("#taiWanId").val()) || $("#hasNeedIdCardSku").val() == "true") {
            if (!$("#tw_sfyz_zone").hasClass("hide") && !$("#noIdcard").hasClass("hide")) {
                var scroll_offset = $('#tw_sfyz_input').offset();
                $("body,html").animate({
                    scrollTop: scroll_offset.top - 80
                });
                $("#idCardNullMessage").addClass("hide");
                $("#idCardErroMessage").addClass("hide");
                $("#idCardUnSaveMessage").removeClass("hide");
                return;
            }
        }
        //港澳售项目
        ...
        if (canSubmitFlag) {
            ...
            if (!$("#paypasswordPanel").is(":hidden")) {
                ...
                if (isEmpty(payPassword)) {
                    // 长密码提示：请输入密码
                    if (!shortPwdActive) {
                        $("#no-pwd-error .error-msg").html("请输入密码");
                    }
                    $("#no-pwd-error").show();
                    return;
                }
                if (shortPwdActive && payPassword != undefined && payPassword.length != 6) {
                    $("#no-pwd-error").show();
                    $("#pwd-error").hide();
                    return;
                }
            ...
            if (domList.length > 0) {
                var venderRemarkCheck = $('#venderRemarkCheck').val();
                var remarks = [];
                for (var i = 0; i < domList.length; i++) {
                    if ($(domList[i]).attr('vendor') != '0') {
                        var remark = {};
                        remark.venderId = $(domList[i]).attr('vendor');
                        var remarkStr = $(domList[i]).find('textarea').val();
                        if (venderRemarkCheck == 0) {
                            remark.remark = order_remark_forbid_trans(remarkStr);
                        } else {
                            if (is_order_remark_forbid(remarkStr)) {
                ...
                success: function (result) {
                    ...
                    if (result.success) {
                        ...
                        if (result.message != null) {
                            ...
                            } else if (result.resultCode == 600215) {
                                var hkDialog = $('body').dialog({
                                    title: '提示',
                                    closeButton: false,
                                    width: 380,
                                    height: 120,
                                    type: 'html',
                                    source: $('#haokaInvalidTip-box').html()
                                });
                                return;
    ...
    function showScaleBuyDialog(result) {
        ...
        var listAfter = '</div><div class="op-btns ar"><a href="#none" onclick="tagSkuScaleBuy()" class="btn-1">继续购买</a><a href="#none" class="btn-9 ml10" onclick="goCart()">返回购物车</a></div>';
                        ...
                        if (venderInsurance.showFlag) {
                            ...
                            if (venderId == "0") {
                                ...
                                if (zythwy != null && zythwy != "" && zythwyDetail != null && zythwyDetail != "") {
                                    ...
                                    $(this).find("span.mode-infor-tips").html("<i id=\"" + venderId + "\" class=\"d-arr d-arr-jd\"></i>" + zythwyDetail);
                                } else {
                                    $(this).find("p.ftx-03").html("自签收后7天内退货，15天内换货，可享1次上门取件服务<a href='#none' class='ftx-05 ml5' id='abcd_" + venderId + "'>查看详情</a>");
                                    $(this).find("span.ftx-01").html("￥" + venderInsurance.insuranceFee.toFixed(2));
                                    $(this).find("span.mode-infor-tips").html("<i id=\"" + venderId + "\" class=\"d-arr d-arr-jd\"></i>自签收后7天内退货，15天内换货，可享1次上门取件服务，此服务不再单独收费。不在京东取件范围内可享受相应运费补偿。");
                                }
                                ...
                                $(this).find("span.ftx-01").html("￥" + venderInsurance.insuranceFee.toFixed(2));
                                $(this).find("span.mode-infor-tips").html("<i id=\"" + venderId + "\" class=\"d-arr d-arr-pop\"></i>自签收后7天内退货，15天内换货，按照赔付标准赔付一次退换货时产生的运费，赔付至京东小金库。");
                            }
...
//无货代下单弹层后处理继续购物
    function continueBuy() {
        ...
        if (isEmpty(skuIds)) {
            ...
            var param = {
                "pin": pin,
                "receiverInfo.receiver": encodeURI(name),
                "receiverInfo.address": encodeURI(fullAddress),
                "receiverInfo.receicerPhoneNo": mobile,
                "validTime": "THREE_MONTH",
                "receiverInfo.area": areaId,
                "skuArray": skuIds,
                "subChannel": 1
            };
            jQuery.ajax({
